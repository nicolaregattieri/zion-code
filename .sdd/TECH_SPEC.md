...
- `Sources/Zion/Views/Main/StatusBar.swift` — at `~line 225-227` (HStack containing `statusBarSectionButton(.code) / .graph / .operations`), append `statusBarSectionButton(.chat)` so Zion Talks gets its footer shortcut alongside Zion Code / Zion Tree / Zion Ops. Update `statusBarSectionLabel(_:)` switch at `~line 321` with `.chat: return L10n("chat.title")` arm if not already exhaustive. Verify icon resolves from `AppSection.chat.icon` (`bubble.left.and.bubble.right`) — already defined in Phase 1 AppEnums update.
